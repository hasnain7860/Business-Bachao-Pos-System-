...
const SalesView = () => {
    ...
    return (
        <>
            <style>{`@media print {body, html { margin: 0; padding: 0; width: 80mm; } .print-container { width: 100%; margin: 0; padding: 0; box-shadow: none; } .no-print { display: none; }}`}</style>
            <div className={`p-4 ${!isPrintMode ? 'bg-gray-100' : ''}`}>
                <div className="no-print flex justify-end mb-4"><button onClick={handlePrint} className="btn btn-primary flex items-center gap-2"><FaPrint /> Print Receipt</button></div>
                <div className="print-container w-72 mx-auto p-3 bg-white text-black shadow-lg font-sans">
                    {userAndBusinessDetail[0]?.business ? (<div className="text-center mb-2"><h2 className="text-xl font-bold">{userAndBusinessDetail[0].business.businessName}</h2><p className="text-xs">{userAndBusinessDetail[0].business.email}</p><p className="text-xs">{userAndBusinessDetail[0].business.phoneNo}</p></div>) : (<p className="text-center text-red-500">Business details not found</p>)}
                    <hr className="my-2 border-dashed border-gray-400" />
                    <div className="text-xs"><div className="flex justify-between"><span>Ref No:</span> <span>{sale.salesRefNo}</span></div>{person && (<div className="flex justify-between"><span>Customer:</span> <span>{person.name}</span></div>)}<div className="flex justify-between"><span>Date:</span> <span>{new Date(sale.dateTime).toLocaleString()}</span></div></div>
                    <hr className="my-2 border-dashed border-gray-400" />
                    <table className="w-full text-xs mt-2">
                        <thead><tr className="border-b border-dashed border-gray-400"><th className="py-1 text-left font-semibold">Item</th><th className="py-1 text-center font-semibold">Qty</th><th className="py-1 text-right font-semibold">Rate</th><th className="py-1 text-right font-semibold">Total</th></tr></thead>
                        <tbody>{sale.products.length > 0 ? (sale.products.map((product, index) => (<tr key={`${product.id}-${index}`}><td className="py-1">{product.name}</td><td className="py-1 text-center">{product.SellQuantity}</td><td className="py-1 text-right">{parseFloat(product.newSellPrice || product.sellPrice).toFixed(2)}</td><td className="py-1 text-right">{(parseFloat(product.newSellPrice || product.sellPrice) * parseInt(product.SellQuantity, 10)).toFixed(2)}</td></tr>))) : (<tr><td colSpan="4" className="text-center py-2">No products</td></tr>)}</tbody>
                    </table>
                    ...
                    <div className="text-xs space-y-1">
                        ...
                        <div className="flex justify-between text-sm font-bold mt-1"><span>Grand Total:</span><span>{currency} {totalBill.toFixed(2)}</span></div>
                    ...
                    {/* --- UPDATED: Smarter and Clearer Account Summary Section --- */}
                    {person && (
                        <>
                            <hr className="my-2 border-t-2 border-dashed border-gray-400" />
                            <div className="text-xs space-y-1">
                                <h4 className="text-center font-bold mb-1 uppercase">Hisab ka Khulasa (Account Summary)</h4>
                                
                                {/* 1. Previous Balance */}
                                {previousBalance > 0 && (
                                    <div className="flex justify-between">
                                        <span className="font-semibold">Pichla Bqaya (Previous Due):</span>
                                        <span>{currency} {previousBalance.toFixed(2)}</span>
                                    </div>
                                )}
                                {previousBalance < 0 && (
                                    <div className="flex justify-between">
                                        <span className="font-semibold">Aapka Advance (Your Advance):</span>
                                        <span>{currency} {Math.abs(previousBalance).toFixed(2)}</span>
                                    </div>
                                )}

                                {/* 2. Current Bill's Due */}
                                <div className="flex justify-between">
                                    <span className="font-semibold">Is Bill ka Bqaya (This Bill's Due):</span>
                                    <span className="font-bold">{currency} {currentCredit.toFixed(2)}</span>
                                </div>
                                ...
                                {netBalance > 0 && (
                                    <div className="flex justify-between text-base font-bold">
                                        <span>Kul Bqaya (Total Due):</span>
                                        <span>{currency} {netBalance.toFixed(2)}</span>
                                    </div>
                                )}
                                {netBalance < 0 && (
                                    <div className="flex justify-between text-base font-bold">
                                        <span>Kul Advance (Total Advance):</span>
                                        <span>{currency} {Math.abs(netBalance).toFixed(2)}</span>
                                    </div>
                                )}
                                {netBalance === 0 && (
                                    <div className="flex justify-between text-base font-bold">
                    ...
                    <div className="text-center text-[10px] text-gray-600"><p className="font-bold">Powered by: Business Bachao</p><p>Developed by: Muhammad Hasnain Tariq</p><p>Contact: 03314460028 (WhatsApp)</p></div>
                </div>
            </div>
        </>
    );
